"""
分析 sr_prompt 对 teacher LLM（如 DeepSeek/Qwen3-API）在 Hotpot QA 上的增益效果，并生成可用于训练 srp_answer 的样本标注。

数据来源：
- 输入：data/srp_prompt/hotpot_train_qa_2000_reprompt.jsonl
...
- 为每条样本打上 direct/sr 是否答对的标签，并输出详细结果 JSON。
...
使用方式（示例）:
    ...
    python src/eval_hotpot_sr_effect_question.py --max_samples 1000 --workers 4 --model deepseek-chat
"""
import argparse
import json
import os
