...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="对比：直接问答 vs 使用 sr_prompt 作为系统提示，在 Hotpot reprompt 数据集上的准确率。"
    )
    parser.add_argument(
        "--model",
        type=str,
        ...
        help="用于评测的 API 模型名称（例如 deepseek-ai/DeepSeek-V3.2）。",
    )
    parser.add_argument(
        "--data_file",
        type=str,
        default="data/reprompt_reson/hotpot_train_qa_2000_reprompt_v2.jsonl",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2000,
        help="最多评测多少条样本（默认 100 条，避免太慢）。",
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
        default=150,
        help="每条样本生成的最大 new tokens（映射到 API 的 max_tokens）。",
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=8,
        help="并行请求的线程数，越大越快但需注意 API 限流，默认 8。",
    )
...
def generate_answer_api(
    ...
    """
    使用 OpenAI 兼容 API（如 SiliconFlow）生成答案。
    """
    ...
    return [
        {
            ...
            "content": (
                ...
                "Answer the user's question with a short phrase or name if possible."
            ),
        },
...
def _eval_one(
    ...
    """供线程池调用：返回 (idx, ok_direct, ok_sr, pred_direct, pred_sr, sample)。"""
...
def main() -> None:
    ...
    # 并行评测，收集 (idx, ok_direct, ok_sr, pred_direct, pred_sr, sample)
    ...
    print("\n===== 结果汇总 =====")
    print(f"总样本数: {total}")
    print(f"直接问答  准确条数: {direct_correct}  准确率: {direct_acc:.3f}")
    print(f"使用sr_prompt 准确条数: {sr_correct}  准确率: {sr_acc:.3f}")
    ...
    for i, ex in enumerate(examples, start=1):
        ...
        print(f"[direct]  pred: {ex['pred_direct']!r}  ok={ex['ok_direct']}")
        print(f"[sr_prompt] pred: {ex['pred_sr']!r}  ok={ex['ok_sr']}")

    print("\n===== 结果不一致的样本（direct vs sr_prompt） =====")
